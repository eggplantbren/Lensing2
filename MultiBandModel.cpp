...
void MultiBandModel::shoot_rays()
{
	const vector< vector<long double> >& x = Data::get_instance().get_x_rays();
	const vector< vector<long double> >& y = Data::get_instance().get_y_rays();
...
void MultiBandModel::calculate_surface_brightness()
{
	for(size_t i=0; i<xs.size(); i++)
	{
		for(size_t j=0; j<xs[i].size(); j++)
...
void MultiBandModel::update_surface_brightness()
{
...
void MultiBandModel::update_rays()
{
	const vector< vector<long double> >& x = Data::get_instance().get_x_rays();
	const vector< vector<long double> >& y = Data::get_instance().get_y_rays();
...
void MultiBandModel::calculate_model_image()
{
