...
95\% posterior probability, is $5.21 M_\odot$. The algorithm has also found
...
\caption{The joint posterior distribution for the SIE mass (integrated within
its critical ellipse, which is not the critical curve of the lens overall),
and the total mass in substructures. The units are square arcseconds (i.e.
a unit mass would have an Einstein radius of one arcsecond).
Assuming a flat cosmology with
$\Omega_m=0.3$, $\Omega_\Lambda=0.7$, and
$H_0=70$ km/s/Mpc, one unit corresponds to 2.07$\times 10^{11} M_\odot$.
\label{fig:masses2}}
\end{center}
\end{figure}
...
and with 75\% probability it is less than 54 units. Clearly, it is not the
substructure mass over the entire domain that is of interest.
