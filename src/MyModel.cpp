...
double MyModel::perturb(RNG& rng)
{
	...
	else if(choice == 2)
	{
        ...
        {
            psf_power += 10.0*rng.randh();
            DNest4::wrap(psf_power, 0.0, 10.0);
...
{
    stringstream s;
    s<<"coeff0, coeff1, correlation_logit, psf_power, ";
    s<<"bg[0], bg[1], bg[2], ";
